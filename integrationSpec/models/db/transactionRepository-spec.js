...
describe('transactionDb wrapper', function () {
    var db;
    var repo;
           ...
           function (done) {
            if(hasEmptyStart(done)) {
                repo.create(objs[1], function (err) {
                    expect(err).toBeNull();
                    repo.getAll(function (err2, transactions) {
           ...
           function (done) {
            if(hasEmptyStart(done)) {
                ...
                repo.update(objids[1], objs[1], function (err) { //}, result) {
                    ...
                    repo.get(objids[1], function (err2, transaction) {
                        expect(err2).toBeNull();
        ...
        it('should delete transactions', function (done) {
            if(hasEmptyStart(done)) {
                repo.delete(objids[0], function (err, result) {
                    ...
                    repo.delete(objids[1], function (err2, result2) {
                        expect(err2).toBeNull();
                        expect(result2.id).toBe(objids[1]);

                        repo.getAll(function (err3, transactions) {
